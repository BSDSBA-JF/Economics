\begin{definition}{Integers ($\mathbb{Z}$)}{integers}
	An integer is an expression of the form $a \minus b$, where $a$ and $b$ are natural numbers.
	Two integers are considered equal ($a \minus b = c \minus d$) if and only if $a + d = c + b.$
\end{definition}
% Problem 1
